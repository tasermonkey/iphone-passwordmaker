...
void ripemd_update(struct ripemd_ctx *ctx, const void *buffer, mutils_word32 len);
void ripemd_final(struct ripemd_ctx *ctx);
void ripemd_digest(struct ripemd_ctx *ctx, mutils_word8 *s);
// md is expected to point to something of the size of RIPEMD160_DIGESTSIZE
// return md.  If md is null returns null
#ifdef __OBJC__
unsigned char* hash160_ripemd(const void* data, mutils_word32 len, unsigned char* md) ;
#else
extern "C" unsigned char* hash160_ripemd(const void* data, mutils_word32 len, unsigned char* md) ;
#endif
