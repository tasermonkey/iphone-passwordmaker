...
void ripemd_update(struct ripemd_ctx *ctx, const void *buffer_in, mutils_word32 len)
{
	mutils_word8* buffer = (mutils_word8*)buffer_in ;
...
unsigned char* ripemd_final2(unsigned char *s, struct ripemd_ctx *ctx) {
	ripemd_final(ctx) ;
...
unsigned char* hash160_ripemd(const void* data, mutils_word32 len, unsigned char* md) {
	...
	ripemd160_init(&ctx);
	ripemd_update(&ctx, data, len) ;
	return ripemd_final2(md, &ctx) ;
}
}